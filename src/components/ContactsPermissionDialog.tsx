...
export const ContactsPermissionDialog: React.FC<ContactsPermissionDialogProps> = ({
  isOpen,
  onAllow,
  onGrant,
  onDeny,
  ...
  const handleConfirm = () => {
    if (onGrant) onGrant();
    else if (onAllow) onAllow();
  };

  return (
    <div
      id="contacts-permission-backdrop"
      className="absolute inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4"
    >
      <motion.div
        id="contacts-permission-card"
      ...
      >
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-center gap-1 text-[10px] font-bold text-teal-500 uppercase tracking-[0.2em]">
            ...
            <span>Android Permission Request</span>
          </div>
          <h3 className="text-base font-bold uppercase tracking-tight text-white">
            Allow Nova to access your contacts?
          </h3>
          <p className="text-xs text-gray-300 leading-relaxed px-2">
            Nova needs the <code className="text-teal-300 font-mono text-[11px] bg-white/5 px-1 py-0.5 rounded">READ_CONTACTS</code> permission to look up phone numbers when you ask to send WhatsApp messages.
          </p>
        </div>

        <div className="p-3 rounded-xl bg-[#161616] border border-white/5 text-[11px] text-gray-400 text-left space-y-1">
          ...
          <p className="text-gray-400 leading-relaxed">Contacts are read locally on your device only during search and never stored on remote servers.</p>
        </div>

        {/* Permission Actions */}
        <div className="flex flex-col gap-2 pt-1">
          <button
            id="allow-contacts-permission-btn"
          ...
          >
            ...
            <span>While using the app (Allow)</span>
          </button>

          <button
            id="deny-contacts-permission-btn"
            onClick={onDeny}
